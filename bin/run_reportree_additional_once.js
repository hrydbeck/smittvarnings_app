...
async function main() {
  console.log(`👀 One-shot: checking for 'additional' R output files in ${watchedFolder}`);
  const pairs = findPairs();
  if (!pairs || pairs.length === 0) {
    console.log("⏳ No R output files detected for additional-samples (one-shot)");
    process.exit(0);
  }
  ...
  for (const { sub, label, profile, meta } of pairs) {
    ...
    await runReportree(metaPath, profilePath, label, sub, { nomenclatureFile: partitions });
  }
}
